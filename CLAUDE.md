...
**現在地（2026-09-08）**：MVP は Google Play の**クローズドテストで配信中**（versionCode 4）。
ほほ笑みラボの生徒さん22人が2026-08-17〜08-25 に参加し、実際に使っている
（8/26 時点で登録23人・回答117件・写真105枚・録音35件）。
**「テスター12人×14日連続」の要件を満たし、製品版へのアクセス申請が可能な状態**。
次にやることは**生徒さんのフィードバックに基づく修正＝チケット 29〜34**（2026-09-08 のヒアリングで確定。
内容は docs/README.md と各チケット）→ **35 製品版申請** → その後 Phase 2（上映会）。
**「ログインなしで使えるようにする」は 2026-09-08 に見送り**（全機能がログイン前提＝Supabase JWT・RLS・worker
の設計で、ログイン前の利用を足すには匿名アカウント方式＝anonymous sign-in＋linkIdentity でも作り直しが要るため。
再び検討するときはこの方式が最小改修）。
...
- **セッション開始時**：docs/README.md と各チケット冒頭のステータス行で現在地を確認する（**README に実装順の例外が書かれていればそちらを優先**）。**00〜28 のチケットはすべて完了済み**＝MVP は Google Play クローズドテスト配信まで到達している。**現在の作業は 29〜35**（クローズドテストの声への修正 → 製品版申請。34 の文言見直しは 29〜33 のあと）。Phase 2（上映会。REQUIREMENTS-PHASE2.md）はその後
...
- **自分史画面（チケット12）**：`app/story.tsx`＋`components/curtain-overlay.tsx`。幕演出はアプリ内でここだけ（DESIGN §7）。CSS アニメは完了コールバックが無いので setTimeout でフェーズ進行し、**開幕は閉幕完了の Promise を await**（429 は幕が閉じ切る前に返る）。生成成功・保存失敗時は本文を捨てず保存だけリトライ。残り回数は「もういちどつくる（あと N 回）」とボタンに表示（詳細は docs/12 メモ）
...
- **`SafeAreaView` の余白は絶対配置の子に効かない**（2026-09-12 実機で確認）。`SkyBackground` は children を `SafeAreaView edges={['top','bottom']}` で包むが、その内側に `position:'absolute'` で置いた要素の `bottom` は**画面の下端からの距離**になる（Android の3ボタンナビに重なった）。画面下に何かを浮かせるときは `useSafeAreaInsets()` の bottom を自分で足すこと（Web は 0 なので二重にならない）
...
- **家族・共有（チケット16）**：`public.redeem_invite_code` RPC が `family_members` への唯一の登録経路（INSERT ポリシーは意図的に無し）。**業務エラーは RAISE せず discriminated jsonb で返す**（`lib/family-join.ts` がパース）。家族の閲覧は専用 `/family/*` ルート（既存画面に readOnly フラグを差し込まない）。見たよは楽観更新＋23505 は成功扱い（`lib/use-my-reactions.ts`）。招待コードは7日・使い捨て・32文字アルファベット（`lib/invite.ts`）
...
- **リリース（チケット23。Google Play 内部テスト配信まで完了）**：
  ...
  - **トラックは2種類**：**内部テスト**＝審査が実質なく数分で配信される。**クローズドテスト**＝審査があり時間はかかる。違いと実績の扱いは下の「製品版への道のり」を参照
...
- **製品版（一般公開）への道のり（2026-08〜09 に実地で判明したこと）**：
  - 要件は「**クローズドテスト**のトラックに**12人以上がオプトインした状態が14日連続**」。内部テストの参加者は**一切カウントされない**（月曜に内部テストで配った生徒さんが数に入らず、アプリ利用22人に対しカウント12人という差が出た）
  - **進捗を見る場所は「ダッシュボード → 製品版 → 製品版へのアクセスの申請」**。ここに「現在、N 人のテスターが M 日間連続でオプトインしています」と出る。Play Console **ホームの「インストール済みユーザー数」は別物**（主に製品版の統計・1〜2日遅れ）で、クローズドテスト中は 0 のままになる＝これを見て焦らない
  - **オプトインの追加は安全・オプトアウトは危険**。テスターを増やす方向（別トラックのリンクを開いてもらう等）はカウントを壊さないが、「テスターとしての登録を解除」やトラックの停止・削除・テスターリストからの除外は**12人を割って連続日数がリセット**されうる。承認されるまでテスト状態を維持する
  - **アプリの更新はクローズドテストの実績を無効にしない**。申請の前後を問わず修正版を配信してよく、「テストの声を受けて直した」は申請時の質問にむしろ有利
  - クローズドトラックは **Alpha** と **生徒テスト** の2本ある（作成経緯の事故）。両方とも同じメーリングリスト（占いキッカケ テスター22人＋自分用1人）・同じ versionCode 4 なので実害はない。**整理は製品版が承認されたあとに行う**
  - テスターへ配るのは各トラックの「テスター数」タブ下部の**オプトイン URL**（「Android で参加」のリンクでよい。「ウェブで参加」はブラウザ経由の代替）
