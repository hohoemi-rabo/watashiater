 ...
 * 机の上ボードの自動整列レイアウト。
 *
 * ── これは `app/lib/board-layout.ts` の**逐語コピー**（チケット21）──
 * 同じ写真・同じ board_seed でアプリと閲覧Webが1ピクセルもずれない配置を出すのが仕事なので、
 * 定数も乱数の引き順もコードの字面も変えない。**片方を直したらもう片方も同じ差分で直すこと**
 * （`diff app/lib/board-layout.ts web/lib/board-layout.ts` がこのヘッダーだけになる状態を保つ）。
 * 共有パッケージにしない理由：3つの独立した npm プロジェクト構成（CLAUDE.md）を
 * このファイル1枚のために崩さない。
 *
 * 閲覧Web側は JS で寸法を測らず、この戻り値を CSS の % に直して使う
 * （left: x*100% / top: (y/H)*100% ＋ transform: translate(-50%,-50%) rotate(Ndeg)。
 * 詳しくは web/components/desk-board.tsx）。
 *
 * ── 座標系の契約（チケット13で確定。14=並べ替え・21=閲覧Web はこれに従う）──
