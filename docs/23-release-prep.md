...
- [x] Google Play Console で内部テスト配信
...
### Play Console の入力内容（2026-08-15 に登録した値）

`docs/store-listing.md` が掲載文・データセーフティの正。ダッシュボードでしか答えられない設問の回答は次のとおり：

- **アプリへのアクセス権**：「一部制限あり」＝はい。手順は「Google ログイン（専用テストアカウント不要・任意の Google アカウントで全機能到達可）」と記載
- **コンテンツレーティング**：カテゴリ「その他すべてのアプリ種類」。ユーザーコンテンツの共有＝はい／**主要なコンテンツソースではない**（見知らぬ他人の投稿に触れる導線が無い）／ヌード・暴力・ブロック機能・報告機能・チャットモデレート＝いいえ／**招待された相手のみに制限できる＝はい**
- **健康アプリ**：「アプリに健康関連の機能はない」／**行政アプリ・金融取引機能**：いいえ／**ターゲットユーザー**：18歳以上
- **カテゴリ**：ライフスタイル（既定の「エンタテイメント」から変更）
- 「難読化解除ファイルがありません」の警告は**無視してよい**（Expo の標準ビルドは R8 難読化をしないためファイル自体が存在せず、クラッシュログは素で読める）

### 将来の申し送り

- **製品版（一般公開）トラックへ進むには、新規個人デベロッパーアカウントは「テスター12人以上×14日間」のクローズドテスト実績が必要**。内部テスト（本チケットの完了条件）には不要
- EAS 無料枠はビルドキュー待ちが発生しうる
- **2回目以降のリリースは `eas submit` で自動化できる**（Google Cloud のサービスアカウント作成が1回だけ必要）。データセーフティ・レーティング・掲載情報はアプリごとに1回きりの入力で、更新時は触らない
