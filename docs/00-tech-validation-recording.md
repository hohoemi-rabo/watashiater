...
- [x] マイク権限の許可フロー（拒否時のやさしい日本語案内を含む）を確認
...
- 許可する側の流れ（初回タップで OS ダイアログ → 許可 → 録音開始）: 実機で確認済み。
  位置情報など他の権限と同じ標準ダイアログで表示され、シニアにも見慣れた形式
- 拒否時の案内（`blocked` 画面・`Linking.openSettings()`）: コードは実装済み。
  Expo Go では Expo Go 自身の権限を操作することになり最終確認にならないため（メモ参照）、
  本番ビルド時（チケット23）に改めて確認する

### metering（チケット10の波形向け）

- `isMeteringEnabled: true` で値が来る：**録音中の波形が声に反応して動くことを実機で確認**。
  チケット10の波形アニメは実データ（metering）駆動で実装できる
