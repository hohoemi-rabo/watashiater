...
- [x] 回答画面・recording-box・写真添付まわりの UI が Web でも操作できることを確認（2026-08-14 PC Chrome・本番URLでユーザー確認。離脱ガードの不具合1件を発見→ `showAlert` で修正済み。メモ参照）
...
2026-08-14、本番URL（PC Chrome）でユーザー確認：録音（許可は押下時のみ・波形が動く）→
保存成功、写真の添付、保存せず「もどる」の確認ダイアログ、写真の「けす」——すべて OK。
Android アプリの回帰も無し。
