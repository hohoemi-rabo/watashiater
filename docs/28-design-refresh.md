...
- [x] じぶん史画面の改装（2026-08-14 追加・ユーザー決定で4案すべて採用）：背景を空グラデに戻し
  「本のページ」カード化（AppCard の地色を story-paper に・木色の枠罫）／扉（中央題字＋ニックネーム＋
  飾り罫。記号グリフ◆）／段落の全角1字下げ（RN は行頭に全角スペース、web は text-indent）／
  奥付（作成日を右寄せで末尾へ。web は `museum.ts` に generated_at を追加取得・JST 明示）。
  閲覧Web のじぶん史の段も同じ体裁（DESIGN §7「Webだけ雰囲気が変わる禁止」）。DESIGN §4・§7 改訂済み
