...
- [x] 実機（Expo Go）で確認：枠タップ→写真選択→追加、5枚で枠が消える、オフライン時は押せない
...
### 文言（2026-09-12 確定。チケット34 で再検討しない）

枠の中は決定事項どおり「ここを押して写真をのせる」。旧「おもいでの写真を のせられます」は
置き換わり、「おもいで」の語は消えたが、**ユーザーが実機で見て「分かりやすい」と確定**した。
34 の表記ルール（日常の漢字・スペースなし・「のせる」はひらがなのまま）にも合っているので、
34 では触らない。
...
**2026-09-12 実機（Expo Go・Android）で動作確認ずみ＝OK**。枠タップからの追加・5枚で枠が消える・
オフライン時に押せないことを確認し、文言もそのまま採用となった。
