...
- [x] 実機（Expo Go）で確認：LINE が直接開き本文が入っている／「ほかの方法で送る」で共有シートが開く
- [x] 追加：補助ボタンの2px枠（アプリ全体）を実機で確認
...
### 補助ボタンに枠を足した（2026-09-12 実機確認で追加。アプリ全体に影響）

LINE ボタンの確認中に、**「見せる用リンクをつくる」が押せるものに見えない**という指摘が出た。
原因はボタンの地 `cardWhite`(#FFFFFF) と `AppCard` の地 `PAPER_TINT`(#FFF9FC) がほぼ同色で、
カードの上では `shadow-raised` だけが頼りになっていたこと。同じことはカード上の補助ボタン全部
（約30か所）に起きていたので、**`SecondaryButton` に文字と同じ色の 2px 枠**を足して一括で直した
（`destructive` は赤枠・赤文字でそろう）。主役ボタンと LINE ボタンは地が濃いので枠は付けない。
DESIGN §5 にルールとして記載。ユーザー選択＝「濃紺の枠 2px」「アプリ全体に適用」。
...
**2026-09-12 実機（Expo Go・Android）で確認ずみ＝OK**。招待コード・見せる用リンクの両方で
LINE が直接開いて本文が入り、「ほかの方法で送る」からは共有シートが開く。緑（#06A845）の濃さ、
補助ボタンの2px枠（全画面）についても調整の指摘は無かった。
